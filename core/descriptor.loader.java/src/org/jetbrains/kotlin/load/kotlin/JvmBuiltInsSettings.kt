...
open class JvmBuiltInsSettings(
    ...
    private fun FunctionDescriptor.getJdkMethodStatus(): JDKMemberStatus {
        ...
        if (DFS.ifAny(
            ...
            javaClassDescriptor ->
            SignatureBuildingComponents.signature(javaClassDescriptor, jvmDescriptor) in DROP_LIST_METHOD_SIGNATURES
        }) return JDKMemberStatus.DROP

        val signature = SignatureBuildingComponents.signature(owner, jvmDescriptor)

        return when (signature) {
            in BLACK_LIST_METHOD_SIGNATURES -> JDKMemberStatus.BLACK_LIST
            in WHITE_LIST_METHOD_SIGNATURES -> JDKMemberStatus.WHITE_LIST
            else -> JDKMemberStatus.NOT_CONSIDERED
        }
    }
