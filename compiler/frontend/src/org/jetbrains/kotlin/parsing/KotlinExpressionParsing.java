...
public class KotlinExpressionParsing extends AbstractKotlinParsing {
     ...
     *   ;
     */
    private boolean parseCallableReferenceSuffix(@NotNull PsiBuilder.Marker expression) {
    ...
    private void parsePostfixExpression() {
        ...
        boolean firstExpressionParsed = at(COLONCOLON) ? parseCallableReferenceSuffix(mark()) : parseAtomicExpression();
        ...
        while (true) {
            if (interruptedWithNewLine()) {
                ...
                if (!parseCallableReferenceSuffix(expression)) {
                    break;
                }
            }
            expression = expression.precede();
